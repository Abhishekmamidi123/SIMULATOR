...
public class onclick {
	public static float execute(int n,String C,int Arr_time[],int Burst_time[],int Priority[],int Time_quantum){
		int i,j;
		float k=0;
...
/*		for (i=0;i<n;i++){
	...
	public static String compare(float A[],String B[],int n,int Arr_time[],int Burst_time[],int Priority[],int Time_quantum){
		int i,j;
		float k;
		Random abhi = new Random();
//		n = abhi.nextInt(10);
...
/*		for (i=0;i<n;i++){
		...
		if(z==0){
			...
			S = "SJF NON-PREMMPTIVE";
		}
		else if(z==3){
			S = "ROUND ROBIN";
		}
		else if(z==4){
			S = "PRIORITY PREEMPTIVE";
		}
		else if(z==5){
			S = "PRIORITY NON-PREMMPTIVE";
		}
		return S;
	}
}